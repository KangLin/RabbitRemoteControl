...
if [ "${BUILD_TARGERT}" = "unix" ]; then
   ...
   export DISPLAY=:99.0
    ...
    sudo dpkg -i ../rabbitremotecontrol*_amd64.deb
    echo "test ......"
    ./test/test_linux.sh
...
if [ ${BUILD_TARGERT} = "android" ]; then
    ${QT_ROOT}/bin/androiddeployqt \
        --input `pwd`/App/android-libRabbitRemoteControl.so-deployment-settings.json \
    ...
    APK_NAME=RabbitRemoteControl_${BUILD_ARCH}_${VERSION}.apk
    ...
    if [ "$TRAVIS_TAG" != "" \
        ...
        sed -i "s/<INFO>.*</<INFO>Release RabbitRemoteControl ${VERSION}</g" update_android.xml
        ...
        sed -i "s:<URL>.*<:<URL>https\://github.com/KangLin/RabbitRemoteControl/releases/download/${VERSION}/${APK_NAME}<:g" update_android.xml
