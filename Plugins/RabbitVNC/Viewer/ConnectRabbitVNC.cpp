...
CConnectRabbitVNC::~CConnectRabbitVNC()
{
    if(m_DataChannel)
        m_DataChannel->close();
    qDebug() << "CConnectRabbitVNC::~CConnectRabbitVNC()";
}
...
int CConnectRabbitVNC::SetChannelConnect(QSharedPointer<CChannel> channl)
{
    bool check = false;
 ...
 * It is not used.
 */
int CConnectRabbitVNC::OnProcess()
{
    return 0;
}
 ...
 * There are not use non-Qt event loop. only use Qt event loop.
 */
void CConnectRabbitVNC::slotTimeOut()
{
    return;
}
